...
# Recon Phase: 03-port-scan

## Workspace
Write all output to the absolute path passed in the prompt
(`~/.dsh/hunts/<domain>/recon/03-port-scan/`). Use absolute paths in your
...
## Phase procedure
Top-1000 TCP port scan against the unique IPs behind in-scope hosts. Prefer naabu; fall back to nmap -sT -Pn --top-ports 1000. Connect scan only. Note shared IPs (multiple vhosts behind one IP).
