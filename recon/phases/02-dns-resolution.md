...
# Recon Phase: 02-dns-resolution

## Workspace
Write all output to the absolute path passed in the prompt
(`~/.dsh/hunts/<domain>/recon/02-dns-resolution/`). Use absolute paths in your
...
## Phase procedure
Forward DNS resolution of in-scope hosts only. Use dig (system). Query A, AAAA, MX, TXT, CNAME, NS, SOA per host. NO PTR lookups. Record anomalies (private IPs, unexpected MX, CNAMEs to third-party providers that could be takeovers).
